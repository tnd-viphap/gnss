...
from libs.rnx2rtkp import RNX2RTKPProcessor
from libs.tps2rin import TPS2RINProcessor
import modules.ftp as ftp
import modules.posfile as pf
import pandas as pd
...
class GNSSProcessor:
    ...
    def process_base_files(self):
        downloader = ftp.FTPDownloader(cfg.FTP_BASE_SETTINGS)
        ...
        base_local_file_paths = downloader.generate_local_file_paths(cfg.BASE_DATA_DIR, base_file_paths)
    ...
    def process_rover_files(self, settings_list):
        for settings in settings_list:
            ...
            downloader = ftp.FTPDownloader(settings)
            rover_file_paths = downloader.get_unprocessed_files_in_remote_path(
                settings["data_dir"], 
                settings["prefix"], 
                rover_data_dir
            )
            rover_local_file_paths = downloader.generate_local_file_paths(rover_data_dir, rover_file_paths)
    ...
    def _process_pos_files(self, output_file_path, output_dir, local_dir,
                          data_rover_east, data_rover_north, data_rover_up):
        pf.create_output_file(output_file_path)
        rtkp_output_log_path = os.path.join(local_dir, "posprocess.txt")
        rtkp_output_file_paths = pf.get_unprocessed_rtkp_output_file_paths(
            output_dir,
    ...
    def _process_rtkp_files(self, file_paths, log_path, output_file,
        ...
        for file_path in file_paths:
            ...
            result = pf.calculate_rtkp_output_file(
                file_path, 
                log_path, 
                data_rover_east,
                data_rover_north,
