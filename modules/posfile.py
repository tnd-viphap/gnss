...
OUTPUT_FILE_HEADERS = ["TIMESTAMP", "Delta_X(mm)", "Delta_Y(mm)", "Delta_Z(mm)"]
...
def create_output_file(file_path):
    ...
    if not output_file_exists or (output_file_exists and output_file.tell() == 0):
        output_file.write(",".join(OUTPUT_FILE_HEADERS) + "\n")
    output_file.close()

# lay cac file chua dc xu ly (chua tao thanh file pos) doi chieu qua file log
def get_unprocessed_rtkp_output_file_paths(output_dir, log_path):
    file_paths = []
...
def calculate_rtkp_output_file(file_path, log_path, east, north, up):
    print(file_path)
    print(log_path)
    ...
    # TODO: remove pos file after processing
    ...
    valid_line_data = []
    invalid_q_counter = 0
    print(invalid_q_counter)
    for line in lines:
        is_q_valid = check_valid_q(line)
        if not is_q_valid:
            invalid_q_counter += 1

        line_data = extract_valid_row(line, east, north, up)
        if line_data:
            valid_line_data.append(line_data)
    print(valid_line_data)
...
def extract_valid_row(row_string, east, north, up):
    if not re.match(r"\d{4}/\d{2}/\d{2} \d{2}:\d{2}:\d{2}", row_string):
        return None
